...
@NamedNativeQuery(name = "Account.calculateProfitLoss", query = "select id,date,details,Dr,Cr,(@balance \\:= @balance + (Dr - Cr)) as Balance from \r\n"
		...
		+ "union all\r\n"
		+ "select concat('CR-',LPAD(receipt_id,4,'0')),date,amount Dr,'' Cr,for_payment_of from receipt where account_id = ?1 and party_id != 1 and (date>=?2 or ?2 is null) and (date<=?3 or ?3 is null) \r\n"
		+ "union all\r\n"
		+ "select concat('CP-',LPAD(cash_voucher_id,4,'0')),date,'' Dr,amount Cr,for_payment_of from cash_voucher where account_id = ?1 and party_id != 1 and (date>=?2 or ?2 is null) and (date<=?3 or ?3 is null) \r\n"
		+ ") GL")
