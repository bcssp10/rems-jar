...
@NamedNativeQueries({
		@NamedNativeQuery(name = "GeneralVoucher.findTrialBalance", query = "select concat('P-',LPAD(party_Id, 2, '0')) id,Account, null as debit,sum(credit) as credit from ( \r\n" + 
				...
				"GROUP BY account_id\r\n" + 
				"union all\r\n" + 
				"select account_id,(select name from account a where a.account_id=c.account_id) Account, sum(amount) from cash_voucher c where (date>=?1 or ?1 is null) and (date<=?2 or ?2 is null) \r\n" + 
				"group by account_id\r\n" + 
				...
				"union all\r\n" + 
				"select account_id,(select name from account a where a.account_id=r.account_id) Account,sum(amount),null from receipt r where (date>=?1 or ?1 is null) and (date<=?2 or ?2 is null) \r\n" + 
